...
class PlayNetwork extends pc.EventHandler {
    ...
    settings.server.on("upgrade", (req, ws, body) => {
        ...
        const callback = (err, data) => {
          if (err || e.msg.id)
            socket.send(
              JSON.stringify({
                name: e.msg.name,
                data: err ? { err: err.message } : data,
                id: e.msg.id,
              })
            );
        };

        if (e.msg.name === "_authenticate")
          return socket.emit("_authenticate", e.msg.data, callback);
      ...
      socket.on("_authenticate", async (payload, callback) => {
        const connectUser = (id) => {
          ...
          if (payload.nickname) {
            console.log("payload.nickname", payload, payload.nickname);
          }
  ...
  async connectRedis(url) {
    ...
    try {
      ...
      throw new Error(
        `Failed to connect to Redis at ${url}, ${err.code}. Ensure it is installed and/or is reachable across the network.`
      );
    }
  ...
  async _onMessage(msg, sender, callback) {
    if (this._reservedEvents.includes(msg.name))
      return callback(new Error(`Event ${msg.name} is reserved`));
    ...
    if (!target) {
      const serverId = parseInt(
        await this.redis.HGET(
          `_route:${msg.scope?.type}`,
          msg.scope.id.toString()
        )
      );
      if (!serverId) return;
  ...
  _validateSettings(settings) {
    ...
    if (!settings.redisUrl) error += "settings.redisUrl is required\n";

    if (!settings.scriptsPath) error += "settings.scriptsPath is required\n";
    ...
    if (
      !settings.server ||
      (!(settings.server instanceof http.Server) &&
        !(settings.server instanceof https.Server))
    )
