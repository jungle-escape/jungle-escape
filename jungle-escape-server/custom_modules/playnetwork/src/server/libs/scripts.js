    ...
    case "json":
      ...
      if (Array.isArray(args.schema)) {
        ...
        for (let i = 0; i < args.schema.length; i++) {
          ...
          if (field.array) {
            result[field.name] = [];
            const arr = Array.isArray(value[field.name])
              ? value[field.name]
              : [];
            ...
            const val = value.hasOwnProperty(field.name)
              ? value[field.name]
              : field.default;
    ...
    case "curve":
      if (value) {
        ...
        if (value instanceof pc.Curve || value instanceof pc.CurveSet) {
          ...
          const CurveType =
            value.keys[0] instanceof Array ? pc.CurveSet : pc.Curve;
...
class Scripts {
  ...
  async initialize(directory) {
          ...
          if (args.array) {
            ...
            if (raw) {
              for (let i = 0, len = raw.length; i < len; i++) {
                this.__attributes[name].push(
                  rawToValue(this.app, args, raw[i], old ? old[i] : null)
                );
              }
            }
          } else {
    ...
    pc.ScriptComponent.prototype._scriptMethod = function (
      script,
      method,
      arg
    ) {
